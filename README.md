...
- **Memory Agent**：使用 DeepSeek-R1 推理模型，多轮调用 25+ 个记忆工具，实时流式输出思考过程和工具调用结果（用户不用干等）
...
### Q: Graphiti 需要什么样的 LLM？

A: 需要支持 **Structured Output** 的 LLM。推荐使用 DeepSeek（支持 JSON mode）或 GPT-4o-mini。实体抽取可以复用你的现有 API Key，支持自定义 base_url。
