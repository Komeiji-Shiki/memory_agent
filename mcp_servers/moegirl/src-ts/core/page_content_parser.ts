...
export class PageContentParser {
  ...
  static parsePage(title: string, content: string): PageStructure {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // 处理标题 - MediaWiki 格式: ==标题==（更宽松的匹配，不要求结尾等号数量相同）
      const headingMatch = trimmedLine.match(/^(={2,6})\s*(.+?)\s*=+\s*$/);
      if (headingMatch) {
        // 保存当前section
        if (currentSection) {
  ...
  static getContentByTitle(structure: PageStructure, title: string): string {
    // 在headings数组中查找匹配的标题
    const heading = structure.headings.find(h =>
    ...
    // 找到下一个同级或更高级的标题
    const headingIdx = structure.headings.indexOf(heading);
    const nextHeadingIndex = structure.headings.findIndex((h, idx) =>
      idx > headingIdx &&
      h.level <= heading.level
    );
    
    const endLine = nextHeadingIndex >= 0
      ? structure.headings[nextHeadingIndex].line
      : Number.MAX_SAFE_INTEGER;
    
    // 收集该标题后的所有sections，直到下一个同级或更高级标题
    const content: string[] = [];
    
    for (const section of structure.sections) {
      // section在标题之后且在下一个标题之前
      if (section.startLine > heading.line && section.startLine < endLine) {
    ...
    // 如果没有内容，检查是否有子标题
    if (content.length === 0) {
      const subHeadings = structure.headings.filter((h, idx) =>
        idx > headingIdx &&
        idx < nextHeadingIndex &&
        h.level > heading.level
      );
      
      if (subHeadings.length > 0) {
        // 返回子标题列表
        const subList = subHeadings.map(h => '  '.repeat(h.level - heading.level - 1) + '• ' + h.title).join('\n');
        return `(该段落包含以下子标题)\n\n${subList}`;
      }
