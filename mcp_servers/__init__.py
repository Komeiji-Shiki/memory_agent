...
def get_available_servers() -> Dict[str, Dict[str, Any]]:
    ...
    for name in os.listdir(MCP_SERVERS_DIR):
        ...
        # 检查服务主文件（stdio 类型可能需要，但不是必须的，比如使用 npx 的服务）
        server_file = os.path.join(server_dir, 'server.py')
        has_server_file = os.path.exists(server_file)
        ...
        servers[name] = {
            "path": server_dir,
            "server_file": server_file if (server_type == "stdio" and has_server_file) else None,
            "enabled": is_enabled,
            "config": config,
...
def generate_mcp_config() -> Dict[str, Any]:
    ...
    for name, info in servers.items():
        ...
        if server_type == "stdio":
            ...
            # 如果有server_file就添加到args开头，否则直接使用配置的args
