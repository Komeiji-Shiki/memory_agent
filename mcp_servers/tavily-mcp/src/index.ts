...
class TavilyClient {
  ...
  private getNextApiKey(): string {
    if (this.apiKeys.length === 0) {
      throw new Error('TAVILY_API_KEY 或 TAVILY_API_KEYS 环境变量未配置');
    }
  ...
  private async postWithApiKey(url: string, data: any, apiKey: string) {
  ...
  private async getWithApiKey(url: string, apiKey: string) {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request: any) => {
      ...
      if (this.apiKeys.length === 0) {
        throw new McpError(
          ...
          "TAVILY_API_KEY 或 TAVILY_API_KEYS 环境变量是必需的。"
        );
      }
      ...
      try {
        ...
        const requestApiKey = this.getNextApiKey();
  ...
  async search(params: any, apiKey: string): Promise<TavilyResponse> {
    try {
      ...
      const response = await this.postWithApiKey(endpoint, cleanedParams, apiKey);
  ...
  async extract(params: any, apiKey: string): Promise<TavilyResponse> {
    try {
      const response = await this.postWithApiKey(
  ...
  async research(params: any, apiKey: string): Promise<TavilyResearchResponse> {
    ...
    try {
      ...
      while (totalElapsed < maxPollDuration) {
        ...
        try {
          const pollResponse = await this.getWithApiKey(
