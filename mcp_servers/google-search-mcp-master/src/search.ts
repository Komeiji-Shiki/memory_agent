    ...
    try {
      ...
      while (retryCount < maxRetries) {
        try {
          ...
          // 如果响应不成功但没有抛出错误（例如429），也进入人工验证流程
          throw new Error(`页面加载状态码: ${response?.status()}`);

        } catch (error) {
          logger.error({ error: error instanceof Error ? error.message : String(error), retry: retryCount + 1 }, "页面加载出错或需要验证");
          
          // 如果是有头模式，等待更长时间以便人工干预
          if (!headless) {
             logger.warn("检测到有头模式，等待人工修复网络或验证码问题（请在弹出的浏览器中完成验证）...");
             // 轮询检查是否通过验证 (最多等待 300 秒)
             ...
             while (waitedTime < maxWaitTime) {
               ...
               const isBlocked = sorryPatterns.some(p => currentUrl.includes(p));
               // 检查是否已经是搜索结果页 (包含 /search?q= 或简单的 /search)
               // 或者只是不再是 blocked 页面
               if (!isBlocked) {
                 logger.info("检测到验证已通过，继续执行...");
                 ...
                 try {
                    if (!noSaveState) {
                        ...
                        logger.info("已保存验证后的浏览器状态");
                    }
                 } catch (e) {
                    logger.error("保存验证状态失败", e);
                 }
                 break; 
               }
             }
      ...
      if (isBlockedPage && !manualVerificationPassed) {
        ...
        if (headless) {
          ...
          if (!manualVerificationPassed) {
              throw new Error("检测到人机验证页面，且在等待时间内未完成验证");
          }
        }
      }
      ...
      // 等待搜索结果加载
      try {
        logger.info("等待搜索结果元素出现...");
        // 尝试等待更广泛的选择器
        await page.waitForSelector("#search, #rso, .g, div[role='main']", {
          timeout: timeout
        });
        ...
        logger.error("无法找到搜索结果元素，尝试截图...");
        const errorScreenshot = path.join(os.tmpdir(), `google-search-no-results-${Date.now()}.png`);
        await page.screenshot({ path: errorScreenshot });
        logger.info({ errorScreenshot }, "已保存错误截图");
        // 不抛出错误，尝试继续提取，看看能拿到什么
      }
      ...
      const results = await page.$$eval(
        "div#search div.g, div#rso > div, div[data-sokoban-container]",
        (elements, maxResults) => {
          return elements
            .slice(0, maxResults)
            .map((el) => {
              ...
              const linkElement = el.querySelector("a[href^='http']");
              
              // 查找摘要：排除标题和链接，找文本内容较长的 div 或 span
              let snippet = "";
              const possibleSnippets = el.querySelectorAll("div, span");
              for (const span of possibleSnippets) {
                  if (span.textContent && span.textContent.length > 20 &&
                      span !== titleElement &&
                      (!linkElement || !linkElement.contains(span))) {
                      // 简单的启发式：如果包含某些标点或长度适中
      ...
      if (results.length === 0) {
          logger.warn("未提取到任何结果，保存页面内容以供调试...");
          const debugHtmlPath = path.join(os.tmpdir(), `google-search-debug-${Date.now()}.html`);
          const html = await page.content();
          fs.writeFileSync(debugHtmlPath, html);
          logger.info({ debugHtmlPath }, "已保存调试HTML");
      }
